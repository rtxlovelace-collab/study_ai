...
* @Description //TODO 
* @File: 7_study_human_interrupt_graph.py
* @Time: 2025/9/15
...
graph_run = graph_builder.compile(checkpointer=check_point, interrupt_before=["tool_exe"])

config = {"configurable":{"thread_id":1}}
state = graph_run.invoke({"messages":[("human","2024年股票指数之怎样的？")]}, config=config)
print(state)

if hasattr(state["messages"][-1], "tool_calls") and len(state["messages"][-1].tool_calls) > 0:
    tool_calls = state["messages"][-1].tool_calls
    print("准备调用工具:", tool_calls)
    human_input = input("请输入yes or no:")
    if human_input.lower() == "yes":
        print(graph_run.invoke(None, config=config))
    else:
        print("Over!")
