...
if __name__ == "__main__":
    app_mem = ConversationSummaryBuffersMemory("",[],250)
    while True:
        ...
        '''
        以优雅的形式打开文件，读取文件内容，并生成摘要
        with open("作业引用.txt", "r", encoding="utf-8") as f:
        world_text = f.read()
        with形式打开文件不用手动关闭txt文件，with语句会自动关闭文件
        相比在传统的手动打开关闭的情况下更加优雅而且
        不会因为在read()方法中出现异常导致文件没有关闭而导致文件句柄泄露
        另外，with语句可以自动捕获异常，并自动关闭文件，
        避免出现意外的错误
        '''
        world_text=open("作业引用.txt", "r", encoding="utf-8")
        answer_prompt =(
            "你是一个很厉害的聊天机器人，请根据上下文给用户一个答案。"
            f"这个是世界观：{world_text.read()}"
