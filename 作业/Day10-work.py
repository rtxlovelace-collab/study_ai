    ...
    def _run(self,*args,**kwargs):
        try:
            ...
            if not gaode_api_key or not gaode_api_url:
                ...
                session.close()
                return f"获取{kwargs.get('IP')}失败"
        except Exception as e:
            return f"获取{kwargs.get('IP')}失败，原因：{e}"
...
morrey=db.similarity_search_with_relevance_scores(query)#根据输入内容的向量表示，在目标向量数据库 / 集合中找到最相似的结果，
                                                        # 并返回这些结果与输入的 “相关性分数”
context=''
for one in morrey:
...
#工具列表
tools=[google, IPqueryTool(), GaodeWeatherTool(),morrey]
# 构建提示词
...
def chat_loop():
    ...
    while True:
        ...
        # 检索相关历史对话
        # print("正在检索相关历史对话...")
        context = morrey.retrieve_relevant_conversations(db, user_input)
        res = agent_executor.invoke({'input': user_input,'context':context})#工具调用
        ai_out = res['output']
        print("AI:", ai_out)
